...
if select_mapacaixa_folder_button:
    ...
    st.session_state.folder = filedialog.askdirectory(title="Select Folder")
    st.session_state.files = os.listdir(st.session_state.folder)
    print(st.session_state.files)
    st.session_state.sheetnames = [f for f in st.session_state.files if re.match(r'[mM].+?\.xlsx$', f)]
    st.session_state.night_sheetnames =  [sheet for sheet in st.session_state.sheetnames if re.search("[nN][oO][iI][tT][eE]", sheet)]
    st.session_state.day_sheetnames = [sheet for sheet in st.session_state.sheetnames if sheet not in st.session_state.night_sheetnames]
...
st.session_state.base_lc_filename = st.file_uploader("Choose a file", type=["xlsx", "csv", "txt"])
if st.session_state.base_lc_filename:
    ...
    st.session_state.new_lc = load_workbook(st.session_state.base_lc_filename, keep_vba=True)
    st.session_state.lc_sheetnames = st.session_state.new_lc.sheetnames
    st.session_state.lc_day_sheetname = [sheet for sheet in st.session_state.lc_sheetnames if "dia" in sheet.lower()][0]
    st.session_state.lc_day_sheet = st.session_state.new_lc[st.session_state.lc_day_sheetname]
    st.session_state.lc_night_sheetname = [sheet for sheet in st.session_state.lc_sheetnames if "noite" in sheet.lower()][0]
    st.session_state.lc_night_sheet = st.session_state.new_lc[st.session_state.lc_night_sheetname]
...
if create_lc_button:
    with st.spinner("Criando LC..."):
        copy_and_paste_values(st.session_state.lc_day_sheet, st.session_state.day_sheetnames, start_copy_cell, end_copy_cell)
        copy_and_paste_values(st.session_state.lc_night_sheet, st.session_state.night_sheetnames, start_copy_cell, end_copy_cell)
